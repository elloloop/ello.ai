...
class ChatGrpcClient {
  ...
  Future<void> init({
    ...
    try {
      ...
      if (_isCloudRun && port == 443) {
        Logger.info('Detected Cloud Run service, applying specialized configuration');
  ...
  void _startConversationAndChat(
    ...
    try {
      ...
      startConversation(clientId: clientId).then((response) {
        if (controller.isClosed) {
          Logger.info('Skipping conversation handling - controller already closed');
          return;
        }
        ...
        Logger.info('Started new conversation with ID: $_currentConversationId');
  ...
  void _sendChatMessage(
    ...
    try {
      ...
      if (message.conversationId.isEmpty) {
        Logger.info('Warning: Message has empty conversation ID, generating one');
        message.conversationId = 'generated-${_uuid.v4()}';
      }

      Logger.info('Sending message to conversation ID: ${message.conversationId}');
      ...
      responseStream.listen(
        (response) {
          ...
          if (response.content.contains("conversation not found") ||
            ...
            Logger.info('Detected conversation not found error in response content');
        ...
        onError: (error) {
          ...
          if (error.toString().contains('Conversation') &&
            ...
            Logger.info('Conversation not found, restarting with new conversation');
    ...
    try {
      ...
      if (_currentConversationId != null &&
        ...
        try {
          ...
          Logger.info('API test failed, but connection might still be good: $e');
  ...
  Future<void> handleConversationNotFound() async {
    ...
    try {
      ...
      if (response.conversationId.isEmpty) {
        Logger.info('Warning: Server returned empty conversation ID during recovery');
        // Generate a client-side ID if server doesn't provide one
  ...
  Future<String> ensureConversationId() async {
    ...
    try {
      ...
      Logger.info('Started new conversation with ID: ${response.conversationId}');
